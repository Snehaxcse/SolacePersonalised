...
export default function QuizQuestion({ question, options, selectedIndex, onSelect, questionIndex }: Props) {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-lg mx-auto px-6">
      <motion.h2
        ...
        className="text-2xl sm:text-3xl font-light text-white/90 text-center mb-12 leading-relaxed"
        style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontWeight: 300 }}
      >
        {question}
      </motion.h2>

      <div className={`grid gap-3 w-full ${isWeatherQuestion ? 'grid-cols-2' : 'grid-cols-1'}`}>
            ...
            >
              {isWeatherQuestion && WeatherIcons[option.label] && (
                <div className="flex flex-col items-center gap-2 py-2">
                  <div className={`transition-colors duration-300 ${isSelected ? 'text-white' : 'text-white/60'}`}>
                    {WeatherIcons[option.label]}
                  </div>
                  <span className={`text-sm font-light transition-colors duration-300 ${isSelected ? 'text-white' : 'text-white/60'}`}>
                    {option.label}
                  </span>
                </div>
              )}
              {!isWeatherQuestion && (
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-sm sm:text-base font-light transition-colors duration-300 ${isSelected ? 'text-white' : 'text-white/70'}`}>
                    {option.label}
                  </span>
                </div>
              )}
